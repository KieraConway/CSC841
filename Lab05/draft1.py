...
scan_complete = Event()         # event - scan operation is complete
...
def weighted_channel_hopper(target_interface, total_duration):
    ...
    try:
        ...
        while (total_duration == 0 or time.time() - start_time < total_duration) and not scan_complete.is_set():
            debug_print(f"[**duration**] {time.time() - start_time}")
